...
module.exports = sevr => function(args) {
	return new Promise((res, rej) => {
		...
		if (args.query) {
			...
			pairs.forEach(pair => {
				const parts = pair.split('=').map(part => { return part.trim().replace(/'/g, "") })
		...
		coll
			...
			.then(docs => {
				...
				.then(result => {
					...
					const filledDoc = fillPopulated(doc, coll.definition.getFields())
					
					this.log(getDocumentOutput(filledDoc, coll.definition.getFields(), '%s'))
					res()
				})
			})
			.catch(err => {
				...
				if (fieldDef.isMultiValue) {
					...
					let valueList = field.reduce((acc, curr) => {
						...
						if (fieldDef.isLinked) {
							item = getDocumentOutput(_.pick(curr, displayField), _.pick(fieldDef.getLinkedCollection().definition.getFields(), displayField), false)
						}else{
					...
					if (fieldDef.isLinked) {
						...
						return getDocumentOutput(_.pick(field, displayField), _.pick(fieldDef.getLinkedCollection().definition.getFields(), displayField), label + ' (%s)')
					}
