...
module.exports = sevr => function(args) {
	return new Promise((res, rej) => {
		...
		const fields = coll.definition.getFields(true)
		let query = {}
		...
		coll
			.read(query, (select.length > 0 && !select.includes(displayField) ? select.concat([displayField]) : select) )
			.then(docs => {
				if (!docs.length) {
			...
			.then(data => {
				...
				const fieldPrompts = Object.keys(fields)
					.filter(key => {
						return data.document.isSelected(key) && (select.length <= 0 || select.includes(key))
					})
					.reduce((prev, key) => {
						return prev.concat(
